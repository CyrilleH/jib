...
`webAppRoot` | `String` | *None* | The root path in the container of the webapp.
`metaInfDirectory` | `Path` | *None* | Adds a file-set to the META-INF dir.
`webInfDirectory` | `Path` | *None* | Adds a file-set to the WEB-INF dir.
...
### Webapp Example with Apache Tomcat

In this configuration, the image:
* Is built from a base of `tomcat:8.5-jre8-alpine` (pulled from Docker Hub)
* Is pushed to `localhost:5000/my-image:webapp-with-jib`
* Runs by calling `catalina.sh run`
* Allows push on an insecure registry
* Has an exploded WAR in `/usr/local/tomcat/webapps/my-webapp`

The default port is `8080` and the webapp context path is `my-webapp`.
...
jib {
  ...
  to {
    image = 'localhost:5000/my-image/webapp-with-jib'
  }
  container {
    webAppRoot = '/usr/local/tomcat/webapps/my-webapp'
